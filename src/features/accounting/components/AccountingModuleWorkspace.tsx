...
function createNextDailyApprovalRecords(
  ...
  return records.map((record) => ({
    ...record,
    id: `${record.module}-approval-${date}`,
    date,
    status: "submitted",
    notes: "Awaiting boss review.",
  }));
}

function createNextDailyDepartmentRecord(
  base: DepartmentModuleState,
  date: string,
): DepartmentModuleState {
  return {
    ...base,
...
function createNextDailyReceptionRecord(
  base: ReceptionModuleState,
  date: string,
): ReceptionModuleState {
  return {
    ...base,
...
function createNextDailyInventoryRecord(
  base: InventoryModuleState,
  date: string,
): InventoryModuleState {
  return {
    ...base,
...
export function AccountingModuleWorkspace({
  ...
  const [selectedApprovalDate, setSelectedApprovalDate] =
  ...
  const vipRecord =
    vipRecords.find((record) => record.id === selectedVipId) ?? vipRecords[0];
  const barRecord =
    barRecords.find((record) => record.id === selectedBarId) ?? barRecords[0];
  const receptionRecord =
    receptionRecords.find((record) => record.id === selectedReceptionId) ??
    receptionRecords[0];
  ...
  const selectedApprovalRecords = approvalRecords.filter(
    (record) => record.date === selectedApprovalDate,
  );

  const deriveApprovalStatus = (records: ApprovalRecord[]): RecordState => {
    if (records.length === 0) return "draft";
    if (records.every((record) => record.status === "fully-resolved")) {
      return "fully-resolved";
    }
    if (records.some((record) => record.status === "partially-resolved")) {
      return "partially-resolved";
    }
    if (records.some((record) => record.status === "approved")) {
      return "approved";
    }
    if (records.some((record) => record.status === "reviewed")) {
      return "reviewed";
    }
    if (records.some((record) => record.status === "submitted")) {
      return "submitted";
    }
    return "draft";
  };

  const approvalDailyStatus = deriveApprovalStatus(selectedApprovalRecords);
  ...
  const resolvedStatusFor = (
    ...
    return debtLedgerService.hasAcceptedUnpaidDebt(moduleKey, date)
      ? "partially-resolved"
      : baseStatus;
  };

  const vipDisplayStatus = resolvedStatusFor(
    vipRecord.status,
    "vip",
    vipRecord.date,
  );
  const barDisplayStatus = resolvedStatusFor(
    barRecord.status,
    "bar",
    barRecord.date,
  );
  const receptionDisplayStatus = resolvedStatusFor(
    receptionRecord.status,
    "reception",
    receptionRecord.date,
  );
  ...
  const createDailyRecord = () => {
    ...
    if (module === "approval") {
      const exists = approvalRecords.some((record) => record.date === date);
      if (exists) {
        setSelectedApprovalDate(date);
        setWarning("A Boss Approval record for today already exists.");
        return;
      }
      ...
      const latestDate =
        [...approvalRecords]
          ...
          .at(-1) ?? todayKey();
      const latestRecords = approvalRecords.filter(
        (record) => record.date === latestDate,
      );
      const next = createNextDailyApprovalRecords(latestRecords, date);
      setApprovalRecords((prev) => [...prev, ...next]);
      setSelectedApprovalDate(date);
      setWarning(null);
    }
  };

  const updateDepartmentRow = (
    index: number,
    field: keyof DepartmentModuleState["rows"][number],
    value: string | number,
  ) => {
  ...
  const updateReceptionRow = (
    index: number,
    field: keyof ReceptionModuleState["rows"][number],
  ...
  const updateInventoryRow = (
    index: number,
    field: keyof InventoryModuleState["purchases"][number],
    value: string | number,
  ) => {
    setInventoryRecords((prev) =>
      prev.map((record) => {
        ...
        if (isLocked(record.status) && role !== "boss") return record;
        const purchases = record.purchases.map((row, rowIndex) =>
          rowIndex === index ? { ...row, [field]: value } : row,
        );
  ...
  const renderDailyHistory = (
    records: {
      id: string;
      date: string;
      status: DepartmentModuleState["status"];
    }[],
    selectedId: string,
    ...
    return (
      <div className="daily-history">
        <div className="daily-history-header">
          <h3>Daily records</h3>
          <button type="button" onClick={createDailyRecord}>
            New day record
          </button>
        </div>
  ...
  const renderDepartment = (title: string, record: DepartmentModuleState) => {
    ...
    const displayStatus = resolvedStatusFor(
      record.status,
      record.module,
      record.date,
    );
      ...
      <div className="workspace-grid">
        ...
        <div className="workspace-toolbar">
          <div className="toolbar-pill">
            Editing {editable ? "enabled" : "locked"}
          </div>
        ...
        <div className="workspace-table-actions">
          <button type="button" onClick={addDepartmentRow} disabled={!editable}>
            + Add Drink Row
          </button>
        </div>
                ...
                <tr key={row.id}>
                  <td>
                    <AutoGrowTextarea
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateDepartmentRow(index, "drinkName", e.target.value)
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <input type="number" value={row.amount} disabled readOnly />
                  </td>
                  <td>
                    <input
                      type="number"
        ...
        <div className="workspace-notes">
          {totals.warnings.length > 0 ? (
            totals.warnings.map((item) => (
              <p key={item} className="warning-text">
                {item}
              </p>
            ))
          ) : (
            <p className="ok-text">No stock mismatches detected.</p>
          )}
        </div>
        ...
        <div className="workspace-summary-grid">
          ...
          <div className="summary-box">
            ...
            <input
              type="number"
              value={record.transfersSection}
              disabled={!editable}
              onChange={(e) =>
                module === "vip"
                  ? setVipRecords((prev) =>
                      prev.map((entry) =>
                        entry.id === selectedVipId
                          ? {
                              ...entry,
                              transfersSection: Number(e.target.value),
                            }
                          : entry,
                      ),
                    )
              ...
              onChange={(e) =>
                module === "vip"
                  ? setVipRecords((prev) =>
                      prev.map((entry) =>
                        entry.id === selectedVipId
                          ? { ...entry, pos: Number(e.target.value) }
                          : entry,
                      ),
                    )
                  : setBarRecords((prev) =>
          ...
          <div className="summary-box">
            <span>Boss collected cash</span>
            <input
              type="number"
              value={record.bossCollectedCash}
              disabled={!editable}
              onChange={(e) =>
                module === "vip"
                  ? setVipRecords((prev) =>
                      prev.map((entry) =>
                        entry.id === selectedVipId
                          ? {
                              ...entry,
                              bossCollectedCash: Number(e.target.value),
                            }
                          : entry,
                      ),
                    )
              ...
              onChange={(e) =>
                module === "vip"
                  ? setVipRecords((prev) =>
                      prev.map((entry) =>
                        entry.id === selectedVipId
                          ? { ...entry, debtsExplanation: e.target.value }
                          : entry,
                      ),
                    )
                  : setBarRecords((prev) =>
        ...
        <div className="workspace-final">
          ...
          <button onClick={submitCurrent} disabled={!editable}>
            Submit record
          </button>
        </div>
      </div>
      ...
      <div className="workspace-grid">
        <div className="workspace-header-row">
          <div>
            ...
            <p>
              Track lodging records, split payments, and multi-day bookings.
            </p>
          </div>
          <div className="workspace-meta">
        ...
        <div className="workspace-table-actions">
          <button type="button" onClick={addReceptionRow} disabled={!editable}>
            + Add Lodging Entry
          </button>
        </div>
                ...
                <tr key={row.id}>
                  ...
                  <td>
                    <input
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "timeIn", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    <input
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "timeOut", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    <AutoGrowTextarea
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "room", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    <AutoGrowTextarea
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "idNumber", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    <input
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "pos", Number(e.target.value))
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <AutoGrowTextarea
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "duration", e.target.value)
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <input
                      type="checkbox"
                      checked={row.existing}
                      disabled={!editable}
                      onChange={(e) =>
                        updateReceptionRow(index, "existing", e.target.checked)
                      }
                    />
                  </td>
                  <td>
        ...
        <div className="workspace-notes">
          {totals.warnings.length > 0 ? (
            ...
            <p className="ok-text">Split payments validation passed.</p>
          )}
        </div>
        ...
        <div className="workspace-summary-grid">
          <div className="summary-box">
            <span>Total sales</span>
            <strong>{formatCurrency(totals.totals.sales)}</strong>
          </div>
          <div className="summary-box">
            <span>Total transfers</span>
            <strong>{formatCurrency(totals.totals.transfer)}</strong>
          </div>
          <div className="summary-box">
            <span>Total POS</span>
            <strong>{formatCurrency(totals.totals.pos)}</strong>
          </div>
          <div className="summary-box">
            <span>Total cash</span>
            <strong>{formatCurrency(totals.totals.cash)}</strong>
          </div>
        </div>

        <div className="workspace-summary-grid">
          <div className="summary-box">
            <span>Debt ledger expense</span>
            <strong>{formatCurrency(debtExpense)}</strong>
          </div>
          <div className="summary-box">
            <span>Net cash after debt</span>
            <strong>{formatCurrency(netCashAfterDebt)}</strong>
          </div>
          <div className="summary-box">
            <span>Debt resolution status</span>
            <strong>{debtExpense > 0 ? "Partially Resolved" : "Clear"}</strong>
          </div>
          <div className="summary-box">
            <span>Ledger source</span>
            <strong>Debt page</strong>
          </div>
        </div>
      ...
      <div className="workspace-grid">
        <div className="workspace-header-row">
          <div>
            ...
            <p>
              Record purchases, manage stock transfers, and flag
              inconsistencies.
            </p>
          </div>
          <div className="workspace-meta">
                ...
                <tr key={row.id}>
                  <td>
                    <AutoGrowTextarea
                      ...
                      disabled={!editable}
                      onChange={(e) =>
                        updateInventoryRow(index, "itemName", e.target.value)
                      }
                    />
                  </td>
                  <td>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateInventoryRow(
                          index,
                          "department",
                          e.target.value as "Store" | "Bar" | "VIP",
                        )
                      }
                    >
                      <option>Store</option>
        ...
        <div className="workspace-table-actions">
          <button type="button" onClick={addInventoryRow} disabled={!editable}>
            + Add Inventory Item
          </button>
        </div>
        ...
        <div className="workspace-summary-grid">
          ...
          <div className="summary-box">
            <span>Transfer to Bar</span>
            <input
              type="number"
              value={inventoryRecord.transfersToBar}
              disabled={!editable}
              onChange={(e) =>
                setInventoryRecords((prev) =>
                  prev.map((entry) =>
                    entry.id === selectedInventoryId
                      ? { ...entry, transfersToBar: Number(e.target.value) }
                      : entry,
                  ),
                )
              }
          ...
          <div className="summary-box">
            <span>Transfer to VIP</span>
            <input
              type="number"
              value={inventoryRecord.transfersToVip}
              disabled={!editable}
              onChange={(e) =>
                setInventoryRecords((prev) =>
                  prev.map((entry) =>
                    entry.id === selectedInventoryId
                      ? { ...entry, transfersToVip: Number(e.target.value) }
                      : entry,
                  ),
                )
              }
          ...
          <div className="summary-box">
            ...
            <strong>{totals.flaggedCount}</strong>
          </div>
        </div>
  ...
  const renderApproval = () => {
    const approvalHistory = approvalDates.map((date) => ({
      ...
      status: deriveApprovalStatus(
        approvalRecords.filter((record) => record.date === date),
      ),
    }));
    ...
    return (
      <div className="workspace-grid">
        <div className="workspace-header-row">
          ...
          <div className="workspace-meta">
            <StatusBadge state={approvalDailyStatus} />
            <span>{getRoleLabel(role)}</span>
          </div>
        </div>

        <div className="workspace-toolbar">
          <div className="toolbar-pill">State: {approvalDailyStatus}</div>
          <div className="toolbar-pill">Date: {selectedApprovalDate}</div>
        </div>

        {renderDailyHistory(
          approvalHistory,
          `approval-${selectedApprovalDate}`,
          (entryId) => {
            const date = entryId.replace("approval-", "");
            setSelectedApprovalDate(date);
          },
        )}
        ...
        <div className="approval-list">
          {selectedApprovalRecords.length === 0 ? (
            <div className="workspace-empty">
              <h2>No records for this day</h2>
              <p>Create a new day record to start boss approvals.</p>
            </div>
          ) : null}
            ...
            <div key={record.id} className="approval-card">
              ...
              <div className="approval-card-grid">
                ...
                <div>
                  ...
                  <strong>{record.debtStatus}</strong>
                </div>
                <div>
                  <span>Cash at hand</span>
                  <strong>{formatCurrency(record.cashAtHand)}</strong>
                </div>
                <div>
                  <span>Notes</span>
