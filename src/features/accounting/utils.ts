import type { DepartmentModuleState, ReceptionModuleState, InventoryModuleState, RecordState } from './types';
...
export function calculateDepartmentTotals(record: DepartmentModuleState) {
  const rows = record.rows.map((row, index) => {
    ...
    const drinksSold = Math.max(totalStock - row.closingStock - row.damages - row.transfers, 0);
    const amount = drinksSold * row.price;
    const warning =
      index < record.rows.length - 1 && record.rows[index + 1].openingStock !== row.closingStock
  ...
  const computedCash = totalSales - record.transfersSection - record.pos - record.bossCollectedCash - totalDamages;
...
export function calculateReceptionTotals(record: ReceptionModuleState) {
  ...
  const warnings = record.rows
    .map((row) => row.amountPaid - row.transfer - row.pos > row.cash ? `${row.customerName}: split payments exceed cash` : '')
    .filter(Boolean);
...
export function calculateInventoryTotals(record: InventoryModuleState) {
  const purchasesTotal = record.purchases.reduce((sum, item) => sum + item.totalCost, 0);
  return {
    purchasesTotal,
