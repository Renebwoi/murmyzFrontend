...
import { getAccessibleModules, getRoleLabel, MODULE_LABELS } from '../features/accounting/permissions';
...
export function AdminLayout() {
  ...
  const accessibleModules = useMemo(() => (user ? getAccessibleModules(user.role) : []), [user]);
  ...
  const navigationItems = [
    ...
    ...accessibleModules.map((module) => ({
      ...
      icon: module === 'vip' ? '🥂' : module === 'bar' ? '🍺' : module === 'reception' ? '🛎️' : module === 'inventory' ? '📦' : '✅',
    })),
  ];
        ...
        <div className="sidebar-footer">
          <div className="user-info">
            ...
            <div className="user-details">
              ...
              <p className="user-role">{user ? getRoleLabel(user.role) : 'Unknown'}</p>
            </div>
          </div>
      ...
      <div className="admin-main">
        <header className="admin-header">
          ...
          <div className="header-right">
            <span className="user-welcome">Welcome, {user?.username} · {user ? getRoleLabel(user.role) : ''}</span>
          </div>
        </header>
